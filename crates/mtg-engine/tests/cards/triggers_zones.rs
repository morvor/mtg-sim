...
fn when_this_land_enters_untapped() {
    ...
    // With three other Mountains, its own "enters tapped unless" doesn't apply, so it
    // enters untapped unless something else puts it onto the battlefield tapped.
    ...
    // Without them it enters tapped (CR 603.6d: part of the event), so it doesn't
    // trigger.
    let mut t = TestGame::new(2);
    let mine = t.hand(P0, "Dwarven Mine");
    let id = put_onto_battlefield(&mut t, P0, mine, false);
    assert!(t.g.obj(id).tapped);
    t.resolve_all();
    assert_eq!(count_subtype(&t, "Dwarf"), 0);
}
